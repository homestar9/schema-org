...
component {
	...
	function init(){
		...
		// The embedded server's webroot is test-harness/. Port 60299 is the canonical test-runner
		// port shared by box.json's testbox runner and the engines that back a test run:
		// adobe@2025, boxlang, and lucee@6 all bind 60299 (only one runs at a time). The other
		// server configs use different ports (lucee@5 = 60300, adobe@2023 = 60301), so start those
		// only for manual, engine-specific checks. Use 127.0.0.1, not localhost: on Windows
		// localhost can resolve to IPv6 ::1 while CommandBox binds the IPv4 loopback, and the runner
		// preflight then times out (408) against a server that is actually up.
		variables.testRunner   = "http://127.0.0.1:60299/tests/runner.cfm";
		...
		variables.excludes = [
			...
			"^[\\/]?build$",
			// Dev-only codegen: generator/ produces types/*.cfc and carries a 1.5MB vocabulary dump.
			// types/ ships (it is the product); the generator that emits it does not. Mirrors box.json ignore.
			"^[\\/]?generator$",
			// Tests + local scratch
			"^[\\/]?test-harness$",
			"^[\\/]?tests$",
			...
			"server-.*\.json",
			"docker-compose.yml",
            // ignore any .code-workspace files, which are VSCode workspace files that are not relevant to the build
            "\.code-workspace$",
	...
	function buildSource(
		...
		// Release branch is "main" (see build/release.boxr, which merges the working branch into main
		// and pushes). Only a build cut from main gets a real build number stamped; everything else is
		// a "-snapshot". The literal must match the actual release branch or releases never get stamped.
		command( "tokenReplace" )
			.params(
				path        = "/#variables.projectBuildDir#/**",
	 ...
	 * branch only influences whether tokenReplace stamps a release number (on "main") or "-snapshot",
	...
	private function ensureTestRunnerReachable(){
		...
		if ( statusCode < 200 || statusCode >= 400 ) {
			return error(
				...
				& "Start a server on port 60299 first (box run-script start:2025) and retry. "
